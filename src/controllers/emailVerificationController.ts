...
const emailVerificationTokens: Map<string, EmailVerificationToken> = new Map();

const generateVerificationToken = (): string => {
  return crypto.randomBytes(32).toString('hex');
};

const storeVerificationToken = (email: string, token: string): void => {
...
const validateVerificationToken = (email: string, token: string): boolean => {
...
const clearVerificationToken = (email: string): void => {
  emailVerificationTokens.delete(email.toLowerCase());
};
...
// @access  Private
const sendEmailVerification = asyncHandler(async (req: Request, res: Response) => {
  try {
